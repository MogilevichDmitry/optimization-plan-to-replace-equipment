...
namespace optimization
{
    class optim
    {
        ...
        static void Main(string[] args)
        {
            ...
            {
                Console.WriteLine("Введите число устройств k-го типа: ");
            ...
            for (int i = 0; i < SumMachines; i++) // add L_k //add MY_ik // add Aikn
            {
                ...
                {
                    Console.WriteLine("Введите время эксплуатации i-го устройства k-го типа: ");
            ...
            {
                tempVn = 0;
                for (int i = 0; i < Mi[k]; i++) // add C_k // Vn 
                {
                    Console.WriteLine("Введите трудоемкость обслуживания одной единицы k-го типа: ");
                    double tempC_k = Double.Parse(Console.ReadLine());
                    C_k.Add(tempC_k);
                    double tempSumAi = 0;
                    for (int q = 0; q < Mi[k]; q++)
                    {
                        tempSumAi += A_ikn[q];
                    }
            ...
            for (int k = 0; k < M; k++) 
            {
                ...
                p3:
                    ...
                    else if (w <= 0)
                    {
                        ...
                        {
                            /* п. 9 */
                            for (int j = 1; j < F_min(t_2k[k], N, n); j++)
                            {
                                /* п.10 */
                                if (B_ikn[n] == 0)
                                {
